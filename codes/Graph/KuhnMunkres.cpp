const int N = 1005;
struct KM { // 0-base, maximum weight matching
...
Usage:
1. graph.init(n);
2. graph.add_edge(a, b, wei);
3. ll ans = graph.solve();
4. for (int i = 0; i < n; ++i) {
    cout << "Matching " << i << " to " << graph.fl[i] << " with weight " << graph.w[i][graph.fl[i]] << endl;
}
if want to find minimum weight matching, just negate the weight of all edges
and then the answer will be the negative of the maximum weight matching
*/